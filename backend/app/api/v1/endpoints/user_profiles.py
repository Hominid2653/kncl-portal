...
@router.get('/{item_id}', response_model=UserProfileResponse, summary='Get UserProfile by id')
async def get_user_profile(item_id: UUID, db: AsyncSession = Depends(get_db)):
    return await service.get(db, item_id)


@router.post('/', response_model=UserProfileResponse, summary='Create UserProfile', status_code=201)
...
@router.patch('/{item_id}', response_model=UserProfileResponse, summary='Update UserProfile')
...
@router.delete('/{item_id}', status_code=204, summary='Delete UserProfile')
