...
async def list_transfer_approval(
    ...
    filters = await authz.scope_transfer_approval_filters(db, current_user, filters)
...
@router.get('/{item_id}', response_model=TransferApprovalResponse, summary='Get Transfer Approval by id')
async def get_transfer_approval(
    item_id: UUID,
    db: AsyncSession = Depends(get_db),
    current_user: CurrentUser = Depends(require_authenticated),
):
    item = await service.get(db, item_id)
    await authz.ensure_can_read_transfer_approval(db, current_user, item)
    return item
...
@router.patch('/{item_id}', response_model=TransferApprovalResponse, summary='Update Transfer Approval')
...
@router.delete('/{item_id}', status_code=204, summary='Delete Transfer Approval')
